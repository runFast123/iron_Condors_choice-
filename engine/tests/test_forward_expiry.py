...
def test_it_books_intrinsic_against_the_closing_spot():
    r = live()
    spot = 23_050.0            # below both lower shorts, above the long wings
    r._settle_and_roll(at(dt.datetime.combine(EXPIRY, dt.time(15, 30))), spot)

    assert all(c.status is CondorStatus.EXPIRED for c in r.condors)
    assert not [c for c in r.condors if c.is_open]
    expected = sum(c.payoff_at_expiry(spot) for c in r.condors)
    assert r.realised == pytest.approx(expected)
    assert "last observed index level" in (r.condors[0].exit_reason or "")
...
def test_the_total_stops_being_zero_once_the_book_has_settled():
    ...
    r._settle_and_roll(at(dt.datetime.combine(EXPIRY, dt.time(15, 30))), 23_050.0)
